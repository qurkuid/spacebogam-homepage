...
- 기존 `.cf-input`과 같은 16px 입력, 직각·ink/paper 버튼, 최소 50px 터치 높이를 쓴다.
- 주소 검색은 전체 너비 버튼 아래 페이지 흐름 안에 삽입한다. 팝업·고정 레이어·내부 스크롤은 만들지 않는다.
- 검색 영역은 SDK가 알리는 높이에 맞춘다. 닫기는 밑줄 버튼이며 키보드 초점을 검색 버튼으로 돌린다.
- 로딩·실패 안내는 `.cf-help`와 live status를 재사용한다. 기본 주소는 검색 선택으로만 입력하며, 검색 실패 시 재시도를 안내한다.
- 상세주소는 주소 바로 아래 선택 항목으로 두고, 주소 선택 후 초점을 넘긴다. 필수 정보 네 가지는 유지한다.
- 선택 항목은 예산·시공 희망일·공사 범위·인테리어 스타일·요청사항을 먼저 펼쳐 보여주고 상담에 도움이 되는 이유를 안내한다. 나머지 항목은 접힘 영역에 유지하며, 미작성해도 접수할 수 있다.
- 주거 예산 구간은 4천만원 미만부터 1억원까지 천만원 단위, 이후 1억~1.5억원·1.5억~2억원·2억원 이상으로 표시하고 선택한 구간 그대로 저장한다.
- 960px 이하 상담 화면은 한 열로 배치해 태블릿에서도 폼의 최소 너비가 가로 넘침을 만들지 않게 한다.
