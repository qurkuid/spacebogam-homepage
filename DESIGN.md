...
- 추천 입력은 `우리 집 계획 (선택)` 하나로 통합한다. 예산 → 공사 범위 → 취향 → 시공 희망일 → 통합 요청사항 순으로, 선택형을 먼저 두고 타이핑은 마지막 한 곳에서 받는다. 안내는 섹션에서 한 번만 하며 미작성해도 접수할 수 있다.
- 예산 금액은 예산 구간으로, 공간별 요청·꼭 반영할 사항·원하는 분위기는 요청사항 하나로 통합한다. 디자인 공사 여부·상담 스타일도 각각 한 번만 묻는다. 대표 질문이 API에 없으면 기존 문항을 유지하고, 통합 답변은 대표 질문 ID로만 저장한다.
- 목적이 다른 시공 희망일·상담 예약일, 생활 방식 질문은 유지한다. 나머지는 `생활 방식·상담 일정 더 알려주기 (선택)`에 접어둔다. 새 단계·모달·필수 조건 없이 모바일 읽기와 타이핑 부담을 줄인다.
- 성공 지표는 폼 시작 대비 추천 항목 완료율(`lead_form_field_complete`)과 접수 완료율(`lead_submit_success`)이다. 입력 수만 늘리고 접수가 줄면 실패로 본다. 전환 개선은 운영 데이터로 확인하기 전에는 주장하지 않는다.
