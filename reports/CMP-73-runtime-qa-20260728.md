...
## P1 — `consultation_submit`은 사이트 코드로 귀속 불가

- 완료 기준은 4개 이벤트에 실험 ID·변형을 연결하도록 요구하지만, `consultation_submit`은 CMP-98에서 오탐(실제 저장 없이 제출 집계) 때문에 클라이언트에서 **의도적으로 제거**됐다. `tests/cmp98-funnel-tracking.test.js:161`이 이를 회귀 검증한다.
- 현재 이 이벤트는 intm.kr 서버측에서 실제 저장 성공 후에만 발생한다. 사이트는 상담 링크에 `experiment_id`/`experiment_variant`를 실어 보내는 데까지만 책임진다(아래 검증 완료).
- 따라서 "저장 성공 후 `consultation_submit`에 실험 ID·변형 연결"은 **intm.kr 측이 해당 파라미터를 이벤트에 이관해야 충족**된다. 사이트 단독으로 닫을 수 없는 항목이다.
