...
(async () => {
  ...
  for (const [route, selector, event, fields] of [
    ['', '[data-v8-event="home_portfolio_cta_click"]', 'home_portfolio_cta_click', ['page_path', 'link_url', 'project', 'is_test']],
    ['portfolio.html', '[data-v8-event="portfolio_project_open"]', 'portfolio_project_open', ['page_path', 'link_url', 'project', 'is_test']],
    ['portfolio.html', '[data-v8-event="portfolio_consult_click"]', 'portfolio_consult_click', ['page_path', 'link_url', 'project', 'is_test']],
    ...
    ['case-daewoo-ian-35py.html', '[data-conversion-event="case_consult_click"]', 'case_consult_click'],
    ['blog.html', '[data-conversion-event="blog_case_open"]', 'blog_case_open'],
    ['blog.html', '[data-conversion-event="blog_consult_click"]', 'blog_consult_click'],
    ['blog.html', '[data-conversion-event="blog_filter_select"]', 'blog_filter_select'],
    ...
    for (const key of fields || ['page_path', 'link_url', 'story_id', 'case_id', 'placement', 'device_class']) assert.ok(Object.hasOwn(hit, key), `${event}: ${key}`);
  }

  console.log('CMP-846 RUNTIME PASS: 12 renders, overflow 0, forbidden CTA/form 0, 10 event samples complete');
  await browser.close();
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
