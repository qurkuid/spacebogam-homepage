...
for (const file of pages) {
  ...
  assert.match(html, /data-no-auto-cta="true"/, `${file}: automatic CTA opt-out`);
  for (const raw of html.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)) JSON.parse(raw[1]);
}

const home = read('index.html');
assert.equal((home.match(/class="v8-story"/g) || []).length, 2, 'home: exactly two approved stories');
assert.equal((home.match(new RegExp(image, 'g')) || []).length, 2, 'home: approved image only');
assert.equal((home.match(/home_blog_story_open/g) || []).length, 2, 'home: story events');
assert.equal((home.match(/home_story_case_open/g) || []).length, 2, 'home: case events');
assert.doesNotMatch(home.match(/<section class="v8-stories"[\s\S]*?<\/section>\s*<\/section>/)?.[0] || '', /href="\/consultation\//, 'home: no story consultation CTA');
